...
namespace Utility
{
    public static class CommonUtilities
    {
        ...
        public static string SaveFile(HttpPostedFileBase file, string savePath)
        {
            ...
            {
                string fileName;
                if (file != null && file.ContentLength > 0 && !string.IsNullOrEmpty(savePath))
                {
        ...
        public static bool DeleteFile(string filePath, string fileName = null)
        {
            if (!string.IsNullOrEmpty(fileName)) filePath = Path.Combine(filePath, fileName);
            try
            {
